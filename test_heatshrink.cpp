// test_heatshrink.cpp : Ce fichier contient la fonction 'main'. L'exécution du programme commence et se termine à cet endroit.
//

#include <iostream>
#include <array>
#include "heatshrink_encoder.h"

int main()
{
	char data[] = {
	 };

	std::array<int8_t, sizeof(data)> dataDelta;
	for (auto i = 0U; i < sizeof(data); i++) {
		dataDelta[i] = i == 0 ? data[i] + 125U : 125U +data[i] - data[i - 1] ;
	}
	uint64_t sunk;

	std::array<uint8_t, 2000> result;
	uint64_t usedSize;
	auto encoder = heatshrink_encoder_alloc(8, 4);
	auto encodeResult = heatshrink_encoder_sink(encoder, reinterpret_cast<uint8_t*>(data), sizeof(data), &sunk);
	auto finishResult = heatshrink_encoder_finish(encoder);
	auto getBufferResult = heatshrink_encoder_poll(encoder, result.data(), result.size(), &usedSize);
	std::cout << "compression finished, started size: " << sizeof(data) << ", compressed size: " << usedSize << std::endl;
}
